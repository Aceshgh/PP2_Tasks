...
class Snake:
    ...
    def check_collision(self, food, CELL):
        ...
        if head.x == food.pos.x and head.y == food.pos.y:
            print(f"Ate food of weight {food.weight}!")
...
class Food:
    ...
    def generate_random_food(self):
        self.pos =Point(random.randint(0, WIDTH//30-1), random.randint(0,HEIGHT//30-1))
        self.weight=random.choice([1,2,3])
        self.spawn_time =pygame.time.get_ticks()
    def isexpired(self,duration=5000):
...
class MenuScene(SceneBase):
    ...
    def Render(self, screen):
        ...
        for i, item in enumerate(self.menu_items):
            text = '+' + item if i == self.active_index else item
...
class GameScene(SceneBase):
    ...
    def Update(self):
        ...
        if self.food.isexpired():
            print("Food disappeared... too slow!")
